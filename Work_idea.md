...
Wine-Intelligence is a wine sourcing intelligence tool built for buyers at restaurants, hotels, and online retailers. The sourcing process today is manual — cross-referencing supplier sheets, critic scores, and purchase history takes time and expertise that not every buyer has.

Wine-Intelligence reduces that to a single gesture. The buyer photographs a wine label. That is the only input. Everything else is computed automatically.
...
| # | What | Details |
| --- | --- | --- |
| 1 | `wine_intelligence.ipynb` | Single notebook, all code and outputs, runs end to end |
| 2 | `wine_intelligence.pdf` | Same notebook exported as PDF |
| 3 | Presentation | 15-20 min slide deck (.pptx or .pdf) |
| 4 | Deployment link | Wine-Intelligence hosted on **Streamlit Cloud** (simpler than FastAPI — no HTML file needed) |

## File structure

```text
wine-intelligence/
├── wine_intelligence.ipynb   ← the submission — fully self-contained
├── README.md             ← this file
├── requirements.txt
├── data/
...
All functions are defined inside `wine_intelligence.ipynb`. No external helper files. The notebook runs end to end on any machine without additional dependencies beyond the pip installs in cell 1.
...
Copy `wine_intelligence.ipynb` to Colab. Switch to the full dataset, set image size to 224x224, train for real. Save weights to Google Drive. Download `cnn_resnet18.pt`, `bilstm.pt`, and `enriched_dataset.csv` back to local.
